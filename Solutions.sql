...
DELIMITER //
CREATE PROCEDURE show_rentals (IN param_ct VARCHAR(40))
BEGIN
SELECT first_name, last_name, email
  FROM customer
  JOIN rental ON customer.customer_id = rental.customer_id
  ...
  JOIN film_category ON film_category.film_id = film.film_id
  JOIN category ON category.category_id = film_category.category_id
  WHERE category.name = param_ct
  GROUP BY first_name, last_name, email;
END
//
DELIMITER ;

CALL show_rentals('Action');
...
DROP PROCEDURE IF EXISTS release_count_category;

DELIMITER //
CREATE PROCEDURE release_count_category (IN param_1 VARCHAR(40))
BEGIN
	SELECT c.name, COUNT(fc.film_id) AS film_count
	FROM category c 
	JOIN film_category fc
	ON fc.category_id = c.category_id
	GROUP BY c.name
    HAVING film_count > param_1;
END
//
DELIMITER ;

CALL release_count_category(70);
